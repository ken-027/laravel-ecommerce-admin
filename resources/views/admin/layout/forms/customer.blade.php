    ...
    <div class="row m-0 p-0 h-100">
        <div class="col-2 p-0 m-0 h-100">
            <ul class="navbar-nav settings bg-main m-0 p-0 pb-2 h-100 position-relative">
                ...
                <li class="">
                    <a class="nav-link px-3" data-content="shippingTab">
        ...
        <div class="col-10 tab-content settings px-0 h-100 form-add-edit" id="formAddEdit" data-editing="customer" style="overflow-x: hidden" data-formtype="{{ $is_edit ? 'edit' : 'add' }}" data-id="{{ $id }}">
            @include('admin.layout.forms.customer.basic')
            @include('admin.layout.forms.customer.shipping-address')
        </div>
    </div>
</div>
